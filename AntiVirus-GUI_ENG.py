...
def port_scan():
    ...
    def portscan(port):
        ...
        try:
            ...
            with print_lock:
                ...
                text_box.insert(END, "[ + ] Port {} is open".format(port) + "\n", "positive")
                text_box.tag_config("positive", foreground="green")
                text_box.see(END)
                text_box.update()
    ...
    text_box.insert(END, "[ + ] Port scan completed in "+str(total) + "\n", "positive")
    text_box.tag_config("positive", foreground="green")
    text_box.see(END)
    text_box.update()
    print("Threader3000 recommends the following Nmap scan:")
    text_box.insert(END, "[ + ] Threader3000 recommends the following Nmap scan:" + "\n", "positive")
    text_box.tag_config("positive", foreground="green")
    text_box.see(END)
    text_box.update()
    text_box.insert(END, "[ + ] nmap -p{ports} -sV -sC -T4 -Pn -oA {ip} {ip}".format(ports=",".join(discovered_ports), ip=myip) + "\n", "positive")
    text_box.tag_config("positive", foreground="green")
    text_box.see(END)
    text_box.update()
...
def button_action_handler(s):
    ...
    if s == "port_scan_button":
        tb = Thread(target=port_scan)
        tb.start()
...
def gui_thread():
    ...
    port_scan_button = tkinter.Button(main, bg=bgc, fg=fgc, text = "Port Scan", command=lambda:button_action_handler("port_scan_button"), height = hoehe, width = breite)
    port_scan_button.grid(row = 2, column = 0)
